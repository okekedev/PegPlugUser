...
import React, { useRef, useState, useMemo, useEffect, forwardRef, useImperativeHandle, useCallback } from 'react';
import { View, Animated, PanResponder, Easing, Dimensions, StyleSheet } from 'react-native';
...
const ONE_TURN = 360;
const LuckyWheel = /*#__PURE__*/forwardRef((props, ref) => {
  ...
  const outerRadius = useMemo(() => props.size / 2 - props.outerRadius, [props.size, props.outerRadius]);
  ...
  const SLICE_PAYLOAD = useMemo(() => {
    ...
    const colors = randomColor({ ...props.backgroundColorOptions,
      count: SLICE_COUNT
    });
  ...
  const knobAnim = Animated.modulo(Animated.divide(Animated.modulo(Animated.subtract(rotate, SLICE_ANGLE_CENTER), ONE_TURN), new Animated.Value(SLICE_ANGLE)), 1);
  ...
  const startSpinning = useCallback(() => {
    ...
    const WINNER_ANGLE_OFFSET = Utils.randomNumber(-SLICE_ANGLE_CENTER + SLICE_ANGLE / 3, SLICE_ANGLE_CENTER - SLICE_ANGLE / 3);
  ...
  const _panResponder = PanResponder.create({
    ...
    onPanResponderMove: (_, gestureState) => {
      ...
      const isClockwiseEnabled = props.gestureType === GestureTypes.CLOCKWISE || props.gestureType === GestureTypes.MULTIDIRECTIONAL;
      const isAntiClockwiseEnabled = props.gestureType === GestureTypes.ANTI_CLOCKWISE || props.gestureType === GestureTypes.MULTIDIRECTIONAL; // set spin velocity
  ...
  const _renderWheel = () => {
    ...
    }, _panResponder.panHandlers), /*#__PURE__*/React.createElement(AnimatedSvg, {
      style: {
        transform: [{
  ...
  }, _renderKnob(), /*#__PURE__*/React.createElement(View, {
    style: {
      transform: [{
