...
import { RANDOM_INITIAL_Y_JIGGLE } from "./constants.js";
...
export const randomXArray = (num, min, max) => {
  'worklet';

  return new Array(num).fill(0).map(() => getRandomValue(min, max));
};
export const generateEvenlyDistributedValues = (lowerBound, upperBound, chunks) => {
  'worklet';

  const step = (upperBound - lowerBound) / (chunks - 1);
  return Array.from({
    length: chunks
  }, (_, i) => lowerBound + step * i);
};
export const generateBoxesArray = (count, colorsVariations, sizeVariations) => {
  'worklet';
  ...
  return new Array(count).fill(0).map(() => ({
    ...
    randomXs: randomXArray(5, -50, 50),
    // Array of randomX values for horizontal movement
...
export const generatePIBoxesArray = (count, colorsVariations, sizeVariations) => {
  'worklet';
  ...
  return new Array(count).fill(0).map(() => ({
    ...
    colorIndex: Math.round(getRandomValue(0, colorsVariations - 1)),
    sizeIndex: Math.round(getRandomValue(0, sizeVariations - 1)),
    randomXs: randomXArray(6, -5, 5),
    // Array of randomX values for horizontal movement
    initialRandomY: getRandomValue(-RANDOM_INITIAL_Y_JIGGLE, RANDOM_INITIAL_Y_JIGGLE),
    initialRotation: getRandomValue(0.1 * Math.PI, Math.PI),
    randomSpeed: getRandomValue(0.9, 1.3),
    // Random speed multiplier
    randomOffsetX: getRandomValue(-50, 50),
    // Random X offset for initial position
    randomOffsetY: getRandomValue(0, 150),
    // Random X offset for initial position
    delayBlast: getRandomValue(0, 0.6),
    // Random velocity multiplier
